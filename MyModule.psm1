...
Import-Module .\Write-ProgressPlus.psm1
...
function Compress-CUEImage
{
    ...
    $cues = Get-ChildItem -LiteralPath $WorkingDirectory -File -Recurse -Filter '*.cue'
    Write-Host "Found $($cues.Count) CUE sheets. Checking...`n"
    $waveCues = foreach ($cue in $cues)
    {
        Write-ProgressPlus -InputObject $cue.Name -TotalCount $cues.count
        $cuePath = $cue | Get-CueAudioPath
        if (-not $cuePath) { continue }
        $folder = Split-Path -Path $cue
        ...
        {
            @{CUE = $cue ; Audio = Join-Path -Path $folder -ChildPath $cuePath; }
        }
    }
    Reset-Progress
    ...
    foreach ($convee in $convertees)
    {
        Write-ProgressPlus -InputObject $convee -TotalCount $convertees.count
        $proc = New-Object -TypeName System.Diagnostics.Process
        $proc.StartInfo.RedirectStandardOutput = $false
        $proc.StartInfo.UseShellExecute = $false
        $proc.StartInfo.FileName = 'flac.exe'
        $proc.StartInfo.Arguments = "-7 `"$($convee.Audio)`""
        $proc.Start() | Out-Null
        $proc.WaitForExit()
        Write-Host "Converted $($convee.Audio) to FLAC" -ForegroundColor Green

        $cuetext = Get-Content -Path $convee.CUE -Raw
        $oldPath = Split-Path -Leaf -Path $convee.Audio
        $newPath = [System.IO.Path]::ChangeExtension($oldPath, 'flac')

        $t = $cuetext.Replace($oldPath, $newPath)
        $t | Out-File -Force -Path $convee.CUE
